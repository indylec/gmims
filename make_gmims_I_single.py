...
ll_full=np.concatenate((ll_q1_map,ll_q2_map,ll_q3_map,ll_q4_map),axis=1)
...
rr_full=np.concatenate((rr_q1_map,rr_q2_map,rr_q3_map,rr_q4_map),axis=1)

ll_full[np.where(ll_full==0)]=np.nan
rr_full[np.where(rr_full==0)]=np.nan

i_plane=0.5*(ll_full+rr_full)
...
#fits.writeto('/local2/scratch/GMIMS/v0.2_data/gmims_I_eq_ch600.fits',i_plane,i_head)

fits.writeto('/local2/scratch/GMIMS/v0.2_data/gmims_RR_eq_ch600.fits',rr_full,i_head)
fits.writeto('/local2/scratch/GMIMS/v0.2_data/gmims_LL_eq_ch600.fits',ll_full,i_head)
