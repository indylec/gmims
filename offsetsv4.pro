...
std=stddev(intercepts)
mean=mean(intercepts)

off_low=mean-1.96*std
off_up=mean+1.96*std
...
pro offsetsv4, gmims_in,haslam_in
...
for i=0,192-1 do begin
   ...
   if (count1 gt 0) and (count2 gt 0) then begin
      ...
      openw,lun,'offsets_slopes_masked_no_boot_v4.txt',/append,/get_lun
      printf,lun,[i,a_b]
      close,lun
      free_lun,lun
