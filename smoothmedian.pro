pro smoothmedian, subfile,averagefile,plotfile,smoothout
  
;**************************************************************************
;Program to further remove median offsets from GMIMS data. Once the
;raw data has been run through the 'subtraction.pro' program, the
;output average map and 'subtracted' map are used as input to this
;program.
;
;The medians of the subtracted data are smoothed with a time-series
;moving average and removed from the data, according to the bin
;boundaries established in subtraction.pro.
;
;The program then outputs the 'smoothed' map (comprising the ~1500
;good channels) and a new averaged map.
;
;
;***************************************************************************
...
map=mrdfits(subfile,0,header)


avgtable=mrdfits(averagefile,2,/use_colnum)
subtable=mrdfits(subfile,1)
...
chunk=map[*,*,start_indices[ii]:end_indices[ii]]
