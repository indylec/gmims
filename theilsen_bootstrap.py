...
def theil_sen(x,y, sample= "auto", n_samples = 1e7, niter=250):
    """
    ...
    for ii in range(niter):
        ...
        for c in xrange(n):
            ...
            intercept=bottleneck.median(intercepts)

        if ii==0:
            np.savetxt('slope_iter0_test.txt',slopes)
            np.savetxt('intercept_iter0_test.txt',intercepts)
    ...
    np.savetxt('slope_bootstrap_250_test.txt',slope_bootstrap)
    np.savetxt('intercept_bootstrap_250_test.txt',slope_bootstrap)
    
    final_slope=bottleneck.median(slope_bootstrap)
    c95=np.percentile(slope_bootstrap,(5,95))
    final_intercept=bottleneck.median(intercept_bootstrap)
    c95i=np.percentile(intercept_bootstrap,(5,95))

    return np.array( [final_slope,final_intercept, c95[0],c95[1],c95i[0],c95i[1]] )
