...
a_b_1=np.zeros((regions,2))
a_b_2=np.zeros((regions,2))
...
for i in range (0,regions):
     a_b_1[i,:]=ts.theil_sen(haslam_div[indices[i],:],gmims0_div[indices[i],:],sample=False)
     # a_b_2[i,:]=ts.theil_sen(gmims0_div[indices[i],:],haslam_div[indices[i],:])
print '...done!'
...
#f=open('coefflist.txt', 'a')

#written=(str(m_hat[0])+' '+str(m_hat[1])+'\n')
